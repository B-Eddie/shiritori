...
function setupGame(io) {
  ...
  function createRoom(socket, { name, avatar, key }) {
    ...
    const room = {
      ...
      players: [],
      settings: { lives: 3, turnSeconds: 15, minLength: 3, maxSkips: 0 },
      game: null,
    };
    rooms.set(code, room);
  ...
  function onTimeout(room) {
    ...
    if (maxSkips > 0) {
      ...
      if (g.consecutiveFails >= maxSkips) {
        ...
        const skip = randomStartChain(room.settings.minLength, g.usedWords);
        g.currentChain = skip.chain;
        g.chainLength = skip.chainLength;
        io.to(room.code).emit("chainSkip", { chain: skip.chain, chainLength: skip.chainLength });
      }
    }
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("updateSettings", (settings) => {
      ...
      if ([0, 2, 3, 4, 5].includes(settings.maxSkips)) s.maxSkips = settings.maxSkips;
      broadcast(room);
    });
